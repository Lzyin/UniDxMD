...
from UniDxMD.data.utils.augmentation_3d import augment_and_scale_3d
...
class A2D2Base(Dataset):
    ...
    categories = {
        ...
        'A2D2_nuScenes': {
            ...
            #                   'Traffic guide obj.', 'Grid structure'],
            # 'ignore': ['Sky', 'Utility vehicle 1', 'Utility vehicle 2', 'Tractor', 'Non-drivable street',
            #            'Blurred area', 'Rain dirt', 'Parking area'],
        }
    }
...
class A2D2SCN(A2D2Base):
    ...
    def __getitem__(self, index):
        ...
        point_depth = data_dict['depth'].copy()
        # intensity = data_dict['reflectance'].copy().reshape(-1, 1) / 255.
        intensity = data_dict['intensity'].copy().reshape(-1, 1) / 255.
        ...
        if np.random.rand() < self.crop_prob:
            ...
            if valid_crop:
                ...
                point_depth = point_depth[keep_idx]
        ...
        depth[img_indices[:, 0], img_indices[:, 1]] = point_depth / 394.
...
def test_A2D2SCN():
    from UniDxMD.data.utils.visualize import draw_points_image_labels, draw_bird_eye_view
