...
from UniDxMD.common.solver.build import build_optimizer, build_scheduler
from UniDxMD.common.utils.checkpoint import CheckpointerV2
from UniDxMD.common.utils.logger import setup_logger
from UniDxMD.common.utils.metric_logger import MetricLogger
from UniDxMD.common.utils.torch_util import set_random_seed
from UniDxMD.models.build import build_model_2d, build_model_3d, build_model_JointPrototypeLearning_add_relation
from UniDxMD.data.build import build_dataloader
from UniDxMD.data.utils.validate import validate
from UniDxMD.models.losses import entropy_loss

from torch import nn
from UniDxMD.models.module.share_codebook import SharedCodebook
...
def train(cfg, output_dir='', run_name=''):
    ...
    best_model_2d_filename = None
    best_model_3d_filename = None
    shared_codebook = SharedCodebook(num = cfg.MODEL_2D.CODEBOOK_NUM, dim = cfg.MODEL_2D.CODEBOOK_DIM).codebook


    # build 2d model
    model_2d, train_metric_2d = build_model_2d(cfg, codebook = shared_codebook)
    ...
    # build 3d model
    model_3d, train_metric_3d = build_model_3d(cfg, codebook = shared_codebook)
    logger.info('Build 3D model:\n{}'.format(str(model_3d)))
    num_params = sum(param.numel() for param in model_3d.parameters())
    print('#Parameters: {:.2e}'.format(num_params))

    # build jointprototype model
    model_joint_pty, train_metric_joint_pty = build_model_JointPrototypeLearning_add_relation(cfg, codebook_dim = cfg.MODEL_2D.CODEBOOK_DIM)
    logger.info('Build joint prototype model:\n{}'.format(str(model_joint_pty)))
    num_params = sum(param.numel() for param in model_joint_pty.parameters())
    print('# prototype model Parameters: {:.2e}'.format(num_params))


    model_2d = model_2d.cuda()
    model_3d = model_3d.cuda()
    model_joint_pty = model_joint_pty.cuda()
    ...
    optimizer_joint_pty = build_optimizer(cfg, model_joint_pty)

    # build lr scheduler
    scheduler_2d = build_scheduler(cfg, optimizer_2d)
    scheduler_3d = build_scheduler(cfg, optimizer_3d)
    scheduler_joint_pty = build_scheduler(cfg, optimizer_joint_pty)
    ...
    checkpointer_joint_pty = CheckpointerV2(model_joint_pty,
                                     optimizer=optimizer_joint_pty,
                                     scheduler=scheduler_joint_pty,
                                     save_dir=output_dir,
                                     logger=logger,
                                     postfix='_joint_pty',
                                     max_to_keep=cfg.TRAIN.MAX_TO_KEEP)
    checkpoint_data_joint_pty = checkpointer_joint_pty.load(cfg.RESUME_PATH, resume=cfg.AUTO_RESUME, resume_states=cfg.RESUME_STATES)
    ...
    best_metric_iter = {'2d': -1, '3d': -1, 'Fuse': -1}
    ...
    def compute_class_prototypes(data, labels, num_classes, pre_class_prototypes):
        valid_indices = (labels >= 0) & (labels < num_classes)
        data = data[valid_indices]
        labels = labels[valid_indices]
        class_sums = torch.zeros(num_classes, data.size(1)).to(data.device)
        class_sums = class_sums.type(data.dtype)
        class_counts = torch.zeros(num_classes).to(data.device)
        
        class_sums.scatter_add_(0, labels.unsqueeze(1).expand_as(data).long(), data)
        class_counts.scatter_add_(0, labels.long(), torch.ones_like(labels).float())
        class_prototypes = class_sums / class_counts.unsqueeze(1)
        if class_counts.min() > 0:
            class_prototypes = class_prototypes
        else:
            missing_classes = (class_counts == 0)
            class_prototypes = class_prototypes.to(dtype=pre_class_prototypes.dtype)
            class_prototypes[missing_classes]   = pre_class_prototypes[missing_classes]
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        optimizer_joint_pty.zero_grad()

        # ---------------------------------------------------------------------------- #
        # Train on source
        # ---------------------------------------------------------------------------- #

        joint_pty, loss_jrc = model_joint_pty(model_joint_pty.memory_bank_2d_src, model_joint_pty.memory_bank_3d_src, model_joint_pty.memory_bank_2d, model_joint_pty.memory_bank_3d)

        train_metric_logger.update(lsr_loss=loss_jrc)


        preds_3d, loss_csqm_3d_src, proj_protytype_3d_src = model_3d(data_batch_src)
        preds_2d, loss_csqm_2d_src, loss_mdd, proj_protytype_2d_src = model_2d(data_batch = data_batch_src, feat_3d = preds_3d['feats'].clone().detach(), prototype_2d = joint_pty.detach(), init=(iteration==0))
        ...
        train_metric_logger.update(loss_csqm_2d_src=loss_csqm_2d_src)
        train_metric_logger.update(loss_csqm_3d_src=loss_csqm_3d_src)


        loss_2d = seg_loss_src_2d
        loss_3d = seg_loss_src_3d

        loss_2d += loss_csqm_2d_src
        loss_3d += loss_csqm_3d_src

        loss_2d += 0.1 * loss_mdd

        train_metric_logger.update(loss_mdd=loss_mdd)
        ...
        # backward
        loss_src = loss_2d + loss_3d + loss_jrc
        ...
        preds_3d, loss_csqm_3d_trg, proj_protytype_3d_trg  = model_3d(data_batch_trg)
        preds_2d, loss_csqm_2d_trg, _, proj_protytype_2d_trg = model_2d(data_batch = data_batch_trg, feat_3d = preds_3d['feats'].clone().detach())

        loss_2d = []
        loss_3d = []

        # vq
        loss_2d.append(loss_csqm_2d_trg)
        loss_3d.append(loss_csqm_3d_trg)

        train_metric_logger.update(loss_csqm_2d_trg=loss_csqm_2d_trg)
        train_metric_logger.update(loss_csqm_3d_trg=loss_csqm_3d_trg)


        ensembl_pred_trg = cfg.TRAIN.XMUDA.lambda_pl_2d * F.softmax(preds_2d["seg_logit"], dim=1) + (1-cfg.TRAIN.XMUDA.lambda_pl_2d) * F.softmax(preds_3d["seg_logit"], dim=1) 
        ensembl_pred_trg = torch.argmax(ensembl_pred_trg, dim=1)
        proj_protytype_2d_trg = nn.functional.normalize(proj_protytype_2d_trg, dim=1)
        proj_protytype_3d_trg = nn.functional.normalize(proj_protytype_3d_trg, dim=1)
        trg_prototype_2d = compute_class_prototypes(proj_protytype_2d_trg, ensembl_pred_trg, num_classes=num_classes, pre_class_prototypes=model_joint_pty.memory_bank_2d)
        trg_prototype_3d=  compute_class_prototypes(proj_protytype_3d_trg, ensembl_pred_trg, num_classes=num_classes, pre_class_prototypes=model_joint_pty.memory_bank_3d)
        src_prototype_2d = compute_class_prototypes(proj_protytype_2d_src, data_batch_src['seg_label'], num_classes=num_classes, pre_class_prototypes=model_joint_pty.memory_bank_2d_src)
        src_prototype_3d=  compute_class_prototypes(proj_protytype_3d_src, data_batch_src['seg_label'], num_classes=num_classes, pre_class_prototypes=model_joint_pty.memory_bank_3d_src)

        model_joint_pty.momentum_update_key_encoder_2d(feat = trg_prototype_2d, init=(iteration==0))
        model_joint_pty.momentum_update_key_encoder_3d(feat = trg_prototype_3d, init=(iteration==0))


        model_joint_pty.momentum_update_key_encoder_2d_src(feat = src_prototype_2d, init=(iteration==0))
        model_joint_pty.momentum_update_key_encoder_3d_src(feat = src_prototype_3d, init=(iteration==0))
        ...
        loss_trg = sum(loss_2d) + sum(loss_3d)
        loss_trg.backward()

        optimizer_2d.step()
        optimizer_3d.step()
        optimizer_joint_pty.step()
...
def main():
    ...
    from UniDxMD.common.config import purge_cfg
    from UniDxMD.config.xmuda import cfg
