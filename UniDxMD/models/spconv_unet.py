...
from UniDxMD.models.module.vq_layers_3d import VectorQuantiser_3d_dense
...
class SpUNetBase(nn.Module):
    def __init__(
        ...
        block = BasicBlock
        self.vq_3d = VectorQuantiser_3d_dense(beta = 0.25, feats_dim= self.channels[4], global_embedding = codebook, num_classes=num_classes, sigma = sigma)
        ...
        self.proj_3d_pty = (
            spconv.SubMConv3d(
                final_in_channels, codebook.embedding_dim, kernel_size=1, padding=1, bias=True
            )
            if num_classes > 0
            else spconv.Identity()
        )
    ...
    def decoder_forward(self, skips):
        ...
        x_feats, vq_loss = self.vq_3d(x.features)
        x = x.replace_feature(x_feats)
        # dec forward
        ...
        x1 = self.final1(x)
        x2 = self.final2(x)
        proj_3d_pty = self.proj_3d_pty(x)

        return x1.features, x2.features, x.features, proj_3d_pty.features, vq_loss
