...
class VectorQuantiser_2d_dense_feature(nn.Module):
    def __init__(self, beta = 0.2, num_classes =None, quantise = 'spatial', feats_dim = None, vq_dim = None, sigma=None, legacy=False):
        super().__init__()
        self.beta = beta
        self.legacy = legacy
        self.quantise = quantise
        self.pre_quant_conv = nn.Conv2d(feats_dim, vq_dim, kernel_size=1,stride=1, padding=0)
        self.p_enc = PositionalEncodingPermute2D(vq_dim)
        self.post_quant_conv = torch.nn.Conv2d(vq_dim, feats_dim, kernel_size=1,stride=1, padding=0)
    ...
    def forward(self, z, embedding):
        ...
        z_tran = self.pre_quant_conv(z)
        z_tran = self.p_enc(z_tran)
        z_tran = rearrange(z_tran, 'b c h w -> b h w c').contiguous()
        z_flattened = z_tran.view(-1, embedding.weight.size(1))
        ...
        dist_normalized = dist_sq / dist_sq.mean()  
        d = torch.exp(-dist_normalized / (2 * self.sigma ** 2))

        num_embeddings_to_select = int(embedding.weight.shape[0] / self.num_classes)
        top_k_values, top_k_indices = torch.topk(d, k=num_embeddings_to_select, dim=1, largest=True)
        ...
        z_q = torch.matmul(weights_masked_normalized, embedding.weight)

        z_q = z_q.view(z_tran.shape)

        loss = self.beta * torch.mean((z_q.detach()-z_tran)**2) + \
                   torch.mean((z_q - z_tran.detach()) ** 2)
