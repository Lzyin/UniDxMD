...
class VectorQuantiser_3d_dense(nn.Module):
    def __init__(self, beta = 0.2, num_classes=None, quantise = 'spatial', sigma = None, legacy=False, feats_dim = None, global_embedding =None):
        ...
        self.pre_quant_mlp = nn.Sequential(
            nn.Linear(feats_dim, feats_dim),
            nn.LeakyReLU(inplace=True),
            nn.Linear(feats_dim, self.global_embedding.embedding_dim))

        self.post_quant_mlp = nn.Sequential(
            nn.Linear(self.global_embedding.embedding_dim, feats_dim),
            nn.LeakyReLU(inplace=True),
            nn.Linear(feats_dim, feats_dim))
    ...
    def forward(self, z):
        z_features = self.pre_quant_mlp(z.float()).float()
        d = (
            torch.sum(z_features ** 2, dim=1, keepdim=True) +
                    torch.sum(self.global_embedding.weight ** 2, dim=1) -
                    2. * torch.matmul(z_features, self.global_embedding.weight.t())
        )

    
        num_embeddings_to_select = int(self.global_embedding.weight.shape[0] / self.num_classes)
        top_k_values, top_k_indices = torch.topk(d, k=num_embeddings_to_select, dim=1, largest=False)
        ...
        row_sums = weights_masked.sum(dim=1, keepdim=True)
        weights_masked_normalized = weights_masked / row_sums

        z_q = torch.matmul(weights_masked_normalized, self.global_embedding.weight)
        z_q = z_q.view(z_features.shape)

        # compute loss for embedding
        if not self.legacy:
            loss = self.beta * torch.mean((z_q.detach()-z_features)**2) + \
                   torch.mean((z_q - z_features.detach()) ** 2)
        else:
            loss = torch.mean((z_q.detach()-z_features)**2) + self.beta * \
                   torch.mean((z_q - z_features.detach()) ** 2)

        z_q = self.post_quant_mlp(z_q.float()).float()
        return z_q, loss
