...
from UniDxMD.models.module.vq_layers_2d import VectorQuantiser_2d_dense_feature


class UNetResNet34(nn.Module):
    def __init__(self, pretrained=True, codebook=None, num_classes = None, sigma = None):
        ...
        self.conv_depth_1 = nn.Conv2d(1, 64, kernel_size=7, stride=1, padding=3, bias=False)
        self.conv_depth_cat = nn.Conv2d(128, 64, kernel_size=1, stride=1)
        ...
        self.quantise = VectorQuantiser_2d_dense_feature(beta=0.25, num_classes=num_classes, feats_dim = 512, vq_dim = self.global_embedding.embedding_dim, sigma = sigma)
    ...
    def forward(self, x, depth):
        ...
        depth = self.conv_depth_1(depth)
        x = self.conv1(x)
        x = torch.cat((x, depth), dim=1)  # Concatenate along the channel dimension
        x = self.conv_depth_cat(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x, q_loss_feature_2d = self.quantise(x, self.global_embedding) 
